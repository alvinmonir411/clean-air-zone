  ...
  webpack: (config: any) => {
    config.plugins.push(
      new webpack.IgnorePlugin({
        resourceRegExp: /^(pg-native|mongodb|hiredis|mysql|mysql2|oracledb|mssql|sql.js|sqlite3|better-sqlite3|ioredis|redis|typeorm-aurora-data-api-driver|pg-query-stream|react-native-sqlite-storage|expo-sqlite|@sap\/hana-client|@google-cloud\/spanner|hdb-pool)$/,
      })
    );
    return config;
  },
